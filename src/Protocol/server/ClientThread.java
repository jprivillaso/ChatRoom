        ...
        try {
            while (true) {
                ...
                if (action.equals("addUser")) {
                    ...
                    ChatServer.usersList.add(nick);
                    response = "addUser&" + nick;
                } else if (action.equals("getUserList")) {
                    String nick = result[1];
                    response = "getUserList&" + nick;
                    for (int i = 0; i < ChatServer.usersList.size(); i++) {
                        if (!nick.equals(ChatServer.usersList.get(i))) {
                            response += "&" + ChatServer.usersList.get(i);
                        }
                    }
                    ...
                    for (int i = 0; i < ChatServer.usersList.size(); i++) {
                        if(ChatServer.usersList.get(i).equals(result[1])){
                            ChatServer.usersList.remove(i);
                            break;
                        }
                    }
                }
